...
class IVF_PQ:
    ...
    def generate_ivf_pq_index(self, database: np.ndarray):
        ...
        self.ivf_centroids, vectors = kmeans2(training_data, self.K, minit='points', iter=self.iterations)
        np.savetxt(self.centroids_path, self.ivf_centroids)
        print("Kmeans IVF Fintio")
        for i in range(self.K):
            ...
            # PQ: Refining each cluster
            pq_centroids = np.zeros((self.M, sub_clusters, self.D_))
            pq_codebook = np.zeros((self.M, cluster.shape[0]), dtype=np.uint32)

            for j in range(self.M):
                pq_centroids[j, :, :], pq_codebook[j, :] = kmeans2(
                    cluster[:, j * self.D_:(j + 1) * self.D_], sub_clusters, minit='points', iter=self.iterations)
